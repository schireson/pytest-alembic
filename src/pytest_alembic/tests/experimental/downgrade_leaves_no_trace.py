...
def test_downgrade_leaves_no_trace(alembic_runner: MigrationContext) -> None:
...
def _test_downgrade_leaves_no_trace(
    connection: Connection, alembic_runner: MigrationContext
) -> None:
...
def check_revision_cycle(
    alembic_runner: MigrationContext, connection: Connection, original_revision: str
) -> None:
    ...
    with connection.begin_nested() as trans:
        ...
        # `upgrade_ops` is Optional on `MigrationScript` in general, but `produce_migrations`
        # always populates it. Asserted rather than assumed so a change upstream surfaces
        # here instead of as an obscure failure inside `render_python_code`.
        assert autogenerated_upgrade.upgrade_ops is not None
        ...
        try:
            ...
            assert autogenerated_post_downgrade.upgrade_ops is not None
